...
- Setting Configuration: Different Approaches - settings_local.py - oldest method. It extends all environment-specific settings in the settings_local.py (ignored by VCS)
- settings.py file:
...
- significance to Configuration -store configuration in environment.
...
- Three different encryption techniques used by SSH: - Symmetric Encryption - Asymmetrical encryption - Hashing
...
- u sed during key exchange algorithm of symmetric encryption.
- one-way hashing - are never to be decrypted; generate unique value of fixed length for each input. -hard to reverse; hard to generate input from hash.
...
- establish SSH connection by: 1.TCP handshake with server 2.ensure secure symmetric connection 3.verify identity on server matches previous records. 4.present user credentials to connect.
